...
- **正式發布模式**：不建立 PyInstaller EXE；改以 `RUN.bat` 搭配專案 `.venv` 啟動，降低辦公室環境的防毒誤判風險。
- **首次啟動自癒**：自動偵測 Python 3.13+、建立 `.venv` 與安裝 `requirements.txt`；已就緒環境改以 `pythonw.exe` 靜默啟動。
- **背景整理**：按「—」縮至 Windows 系統列並維持整理；按「×」或系統列選單結束時，仍會先完成目前檔案的安全處理。
