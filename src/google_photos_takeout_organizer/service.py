...
def analyze(inputs: list[Path], work: Path, cancel_requested: Callable[[], bool] | None = None) -> dict:
    ...
    manifest = {"schema_version":"0.1", "app_version":"1.2.0", "dataset_id":str(uuid.uuid4()), "input_archives":archives, "media_records":[r.to_dict() for r in records], "duplicate_groups":[g.to_dict() for g in groups], "summary":summary, "warnings":warnings, "errors":[], "export_state":"PLANNED"}
